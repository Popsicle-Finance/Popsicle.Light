<template>
  <div class="modal" v-if="pool" @click.stop v-click-outside="closeModal">
    <img
      ...
      alt="close"
    />
    <h3 class="title">Stake from your LP position</h3>
    <div class="input-wrapper">
      <UpSliderInput
        :name="'PLP'"
        :imageName="pool.stakingTokenIcon"
        :userBalance="pool.lpBalance"
        :error="amountError"
        :parentValue="amount"
    ...
    <div class="btn-wrap border width-100" :class="{ disabled: isBtnDisabled }">
      <button class="btn width-100" @click="depositHandler">
        {{ actionBtnText }}
      </button>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    amountError() {
      if (+this.amount && +this.amount > +this.pool.lpBalance) {
        return `The value cannot be more than ${this.pool.lpBalance}`;
      }
    ...
    actionBtnText() {
      if (!this.pool.allowance) return "Approve LP Spending";
      return "Stake";
    },
    account() {
  ...
  methods: {
    ...
    async depositHandler() {
      ...
      try {
        const amount = this.$ethers.utils.parseUnits(String(this.amount), 18);
